...
export default function MyEquipmentPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto text-xs">
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-500">Loading your equipment portfolio...</div>
      ) : (
        <div className="space-y-6">

          {/* Section 1: My Equipment Requests */}
          <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4 shadow-md">
            ...
            {(!myEquipmentData.requests || myEquipmentData.requests.length === 0) ? (
              <div className="p-6 text-center text-slate-500 bg-slate-50/50 border border-slate-200 rounded-xl space-y-2">
                <FileText className="w-8 h-8 text-slate-400 mx-auto" />
                <p className="font-semibold text-slate-700">No equipment requests submitted yet.</p>
                <p className="text-[11px] text-slate-400">Click "Request Equipment" above to submit a request for your shoot project.</p>
              </div>
            ) : (
                          ...
                          <span
                            className={`px-2 py-0.5 rounded font-extrabold text-[10px] uppercase border ${
                              r.status === 'APPROVED' || r.status === 'CHECKED_OUT'
                                ...
                                : 'bg-gray-500/20 text-slate-700 border-gray-500/40'
                            }`}
                          >
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4 shadow-md">
            ...
            {(!myEquipmentData.movements || myEquipmentData.movements.length === 0) ? (
              <div className="text-xs text-slate-500 p-4 text-center">No active or past equipment handovers recorded for your account.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                  ...
                  <tbody className="divide-y divide-border/60">
                    {(myEquipmentData.movements || []).map((m: any) => (
                      <tr key={m.id} className="hover:bg-slate-100/40">
                        ...
                        <td className="p-3 font-bold text-purple-700">{m.project?.name || 'Shoot Project'}</td>
                        <td className="p-3 font-mono text-slate-700">
                        ...
                        <td className="p-3 text-slate-700">
                          <span className="px-2 py-0.5 rounded font-extrabold text-[10px] uppercase bg-blue-50 text-blue-700 border border-blue-200 mr-2">
                            {m.action}
                          </span>
                          {m.notes}
                        </td>
      ...
      {showRequestModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-lg rounded-2xl border border-slate-200 p-6 space-y-4 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                <Camera className="w-5 h-5 text-emerald-600" />
                Submit Equipment Request
              </h3>
              <button onClick={() => setShowRequestModal(false)} className="text-slate-500 hover:text-slate-900">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateRequest} className="space-y-4 text-xs">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Select Assigned Shoot Project *</label>
                <select
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Select Equipment Item *</label>
                <select
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Expected Return Date *</label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Purpose & Requirements *</label>
                <textarea
                  required
                  rows={2}
