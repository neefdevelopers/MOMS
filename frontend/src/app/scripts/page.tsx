  ...
  return (
    <div className="space-y-6 text-xs">
      <div className="bg-white border border-slate-200 p-6 rounded-xl flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <FileText className="w-5 h-5 text-blue-600" /> Media Scripts Repository
          </h1>
        </div>
      ...
      {(user?.role === 'TECHNICAL_MANAGER' || (user?.role as string) === 'ADMINISTRATOR' || (user?.role as string) === 'ADMIN') && (
        <div className="p-4 rounded-xl bg-blue-50 border border-blue-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-blue-100 text-blue-700 border border-blue-200 flex items-center justify-center shrink-0 font-bold text-lg">
              
            </div>
            <div>
              <h4 className="text-sm font-bold text-blue-900">Technical Manager Script Review Session</h4>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-blue-100 text-blue-800 border border-blue-200 font-mono font-bold">
      ...
      {(user?.role === 'MEDIA_MANAGER' || (user?.role as string) === 'ADMINISTRATOR' || (user?.role as string) === 'ADMIN') && (
        <div className="p-4 rounded-xl bg-cyan-50 border border-cyan-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-cyan-100 text-cyan-700 border border-cyan-200 flex items-center justify-center shrink-0 font-bold text-lg">
              
            </div>
            <div>
              <h4 className="text-sm font-bold text-cyan-900">Media Manager Script Session</h4>
            </div>
          </div>
          <button
            type="button"
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 shadow-xs ${
              filterStatus === 'WAITING_FOR_MEDIA_REVIEW'
                ...
                : 'bg-white hover:bg-cyan-100 text-cyan-800 border border-cyan-200'
            }`}
          >
            <span>Pending Media Manager Approval</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-cyan-100 text-cyan-800 border border-cyan-200 font-mono font-bold">
      ...
      {(user?.role === 'MARKETING_MANAGER' || (user?.role as string) === 'ADMINISTRATOR' || (user?.role as string) === 'ADMIN') && (
        <div className="p-4 rounded-xl bg-purple-50 border border-purple-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-purple-100 text-purple-700 border border-purple-200 flex items-center justify-center shrink-0 font-bold text-lg">
              
            </div>
            <div>
              <h4 className="text-sm font-bold text-purple-900">Marketing Manager Script Approval Session</h4>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-purple-100 text-purple-800 border border-purple-200 font-mono font-bold">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3 overflow-x-auto">
        ...
        >
          ...
          <span className="px-2 py-0.5 rounded-full text-[10px] bg-blue-50 text-blue-700 border border-blue-200 font-mono font-bold">
            {scripts.length}
          </span>
        </button>
        ...
        >
          ...
          <span className="px-2 py-0.5 rounded-full text-[10px] bg-cyan-50 text-cyan-700 border border-cyan-200 font-mono font-bold">
            {scripts.filter((s) => s.status === 'WAITING_FOR_MEDIA_REVIEW').length}
          </span>
        </button>
        ...
        >
          ...
          <span className="px-2 py-0.5 rounded-full text-[10px] bg-purple-50 text-purple-700 border border-purple-200 font-mono font-bold">
            {scripts.filter((s) => ['PENDING_MARKETING_APPROVAL', 'WAITING_FOR_MARKETING_APPROVAL', 'DRAFT', 'PENDING_APPROVAL', 'CHANGES_REQUESTED'].includes(s.status)).length}
          </span>
        </button>
      ...
      <div className="bg-white border border-slate-200 p-5 rounded-xl space-y-4 text-xs shadow-md">
        ...
        {(filterClient !== 'ALL' || filterProject !== 'ALL' || filterBrand !== 'ALL' || filterProduct !== 'ALL' || filterLanguage !== 'ALL' || filterCategory !== 'ALL' || filterEmployee !== 'ALL' || filterStatus !== 'ALL' || filterPriority !== 'ALL' || filterDate) && (
          <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-slate-200">
            ...
            {filterClient !== 'ALL' && (
              <span className="px-2.5 py-1 bg-pink-50 text-pink-700 border border-pink-200 rounded-full flex items-center gap-1 text-[11px]">
                Client: {clientsList.find((c) => c.id === filterClient)?.name}
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterClient('ALL')} />
              </span>
            )}
            {filterBrand !== 'ALL' && (
              <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full flex items-center gap-1 text-[11px]">
                ...
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterBrand('ALL')} />
              </span>
            )}
            {filterProduct !== 'ALL' && (
              <span className="px-2.5 py-1 bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-full flex items-center gap-1 text-[11px]">
                ...
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterProduct('ALL')} />
              </span>
            )}
            {filterLanguage !== 'ALL' && (
              <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-full flex items-center gap-1 text-[11px]">
                ...
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterLanguage('ALL')} />
              </span>
            )}
            {filterCategory !== 'ALL' && (
              <span className="px-2.5 py-1 bg-amber-50 text-amber-800 border border-amber-200 rounded-full flex items-center gap-1 text-[11px]">
                ...
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterCategory('ALL')} />
              </span>
            )}
            {filterStatus !== 'ALL' && (
              <span className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full flex items-center gap-1 text-[11px]">
                Status: {filterStatus}
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterStatus('ALL')} />
              </span>
            )}
            {filterPriority !== 'ALL' && (
              <span className="px-2.5 py-1 bg-rose-50 text-rose-700 border border-rose-200 rounded-full flex items-center gap-1 text-[11px]">
                Priority: {filterPriority}
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterPriority('ALL')} />
              </span>
            )}
            {filterDate && (
              <span className="px-2.5 py-1 bg-slate-100 text-slate-800 border border-slate-200 rounded-full flex items-center gap-1 text-[11px] font-mono">
                Date: {filterDate}
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterDate('')} />
              </span>
            )}
            {filterEmployee !== 'ALL' && (
              <span className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-full flex items-center gap-1 text-[11px]">
                Staff: {usersList.find((u) => u.id === filterEmployee)?.name}
                <X className="w-3 h-3 cursor-pointer hover:text-slate-900" onClick={() => setFilterEmployee('ALL')} />
              </span>
            )}
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-50/70 p-3.5 rounded-xl border border-slate-200 space-y-2.5">
                <div className="font-bold text-blue-700 text-[11px] uppercase tracking-wider flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5 text-blue-600" /> Crew &amp; Personnel
                </div>
                <div className="space-y-2">
                  <select
              ...
              <div className="bg-slate-50/70 p-3.5 rounded-xl border border-slate-200 space-y-2.5">
                <div className="font-bold text-amber-800 text-[11px] uppercase tracking-wider flex items-center gap-1.5">
                  <Filter className="w-3.5 h-3.5 text-amber-600" /> Status, Priority &amp; Date
                </div>
                <div className="space-y-2">
                  <select
                    value={filterStatus}
                  ...
                  <div>
                    <label className="block text-slate-500 font-semibold mb-1 text-[10px]">Filter Date</label>
                    <input
                      type="date"
                      value={filterDate}
        ...
        <div className="flex items-center justify-between text-[11px] pt-2 border-t border-slate-200">
          <span className="text-slate-500 font-medium">
            Showing <strong className="text-blue-600 font-bold">{filteredScripts.length}</strong> of <strong className="text-slate-900">{scripts.length}</strong> scripts
          </span>
        </div>
      </div>

      {loading ? (
        <div className="p-8 text-center text-slate-500">Loading Scripts Repository...</div>
      ) : filteredScripts.length === 0 ? (
        <div className="p-12 text-center bg-slate-50 border border-slate-200 rounded-xl space-y-2">
          <p className="text-slate-500 font-semibold text-sm">No scripts match your search criteria</p>
          <p className="text-slate-400 text-xs">Try clearing search filters or searching for a different keyword.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="space-y-2">
                  <div className="flex justify-between items-start font-mono text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase border ${
                        s.priority === 'CRITICAL' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-amber-50 text-amber-800 border-amber-200'
                      }`}>
                        {s.priority}
                      </span>
                  ...
                  <h3 className="font-bold text-slate-900 text-base font-mono leading-snug">{s.name}</h3>

                  <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-700 bg-slate-50/60 p-3 rounded-lg border border-slate-200">
                    <div><span className="text-slate-400">Project:</span> <strong className="text-slate-900">{s.project?.name || s.project?.projectId}</strong></div>
                    <div><span className="text-slate-400">Client:</span> <strong className="text-slate-800">{s.client?.name}</strong></div>
                    <div><span className="text-slate-400">Brand:</span> <strong className="text-purple-600">[{s.brand?.shortCode}] {s.brand?.name}</strong></div>
                    <div><span className="text-slate-400">Product:</span> <strong className="text-emerald-600">{s.product?.name || 'N/A'}</strong></div>
                    <div><span className="text-slate-400">Campaign:</span> <strong className="text-indigo-700">{s.campaign?.name || 'N/A (Optional)'}</strong></div>
                  ...
                  {s.objective && (
                    <div className="text-[11px] text-slate-700">
                      <span className="text-slate-400 font-semibold">Objective:</span> {s.objective}
                    </div>
                  )}
                  ...
                  {s.description && (
                    <div
                      ...
                      className="text-[11px] text-purple-700/90 bg-purple-50 border border-purple-200 p-2.5 rounded-lg cursor-pointer hover:border-purple-500 transition-all group space-y-1"
                    >
                      <div className="flex items-center justify-between text-[10px] text-purple-600 font-bold">
                        <span className="flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5" />
                          <span>Script Storyline &amp; Scenes</span>
                        </span>
                        <span className="group-hover:underline text-purple-700">Read Full Popup</span>
                      </div>
                      <p className="line-clamp-2 text-slate-700 font-sans leading-relaxed">
                        {s.description}
                      </p>
                    </div>
                  )}
                  ...
                  {s.remarks && (
                    <div className="text-[10px] text-amber-800 bg-amber-50 border border-amber-200 p-2 rounded">
                      <strong className="text-amber-600">Remark:</strong> "{s.remarks}"
                    </div>
                  )}
                </div>

                <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500 gap-2 flex-wrap">
                  <span>Created by: <strong className="text-slate-800">{s.createdBy?.name || 'Writer'}</strong></span>
                  ...
                  {s.status === 'WAITING_FOR_TECHNICAL_REVIEW' && (user?.role === 'TECHNICAL_MANAGER' || (user?.role as string) === 'ADMINISTRATOR' || (user?.role as string) === 'ADMIN') ? (
                    <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className="px-2 py-1 bg-rose-50 hover:bg-red-600/30 border border-rose-200 text-rose-700 font-bold rounded text-[10px] flex items-center gap-1"
                      >
                        Reject
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className="px-2 py-1 bg-amber-50 hover:bg-amber-600/30 border border-amber-200 text-amber-800 font-bold rounded text-[10px] flex items-center gap-1"
                      >
                        Revision
                      </button>
                    </div>
                  ) : (
                    <span className="text-[10px] font-mono text-slate-400">Click to View Details</span>
                  )}
                </div>
              </div>
            );
      ...
      {selectedScript && (() => {
        ...
        return (
          ...
          <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl max-w-2xl w-full space-y-4 max-h-[90vh] overflow-y-auto text-xs">
            <div className="flex justify-between items-center border-b border-slate-200 pb-3">
                ...
                <h2 className="text-base font-bold text-slate-900 mt-1 font-mono">{selectedScript.name}</h2>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
                  <div>
                    ...
                    <strong className="text-blue-600 font-mono text-xs">{selectedScript.scriptId}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 text-[10px] uppercase font-bold block">Script Name</span>
                    <strong className="text-slate-900 font-mono text-xs block truncate">{selectedScript.name}</strong>
                  </div>
                  <div>
                    <span className="text-slate-400 text-[10px] uppercase font-bold block">Shoot Project</span>
                    <strong className="text-slate-800 text-xs block truncate">{selectedScript.project?.name || 'N/A'}</strong>
                  </div>
                  <div>
                ...
                <div className="flex justify-between items-center pt-2.5 border-t border-slate-200 text-xs">
                  <span className="text-slate-500 text-[11px]">
                    Assigned Staff: <strong className="text-amber-800">{selectedScript.scriptAssignments?.map((a: any) => a.user?.name).filter(Boolean).join(', ') || 'Not Assigned'}</strong>
                  </span>
                  <button
                    type="button"
                ...
                {showFullDetails && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-200 text-[11px] animate-in fade-in duration-150">
                    <div><span className="text-slate-400 block text-[10px]">Client:</span> <strong className="text-slate-800">{selectedScript.client?.name || 'N/A'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Brand:</span> <strong className="text-purple-600">[{selectedScript.brand?.shortCode}] {selectedScript.brand?.name}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Product:</span> <strong className="text-emerald-600">{selectedScript.product?.name || 'N/A'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Campaign:</span> <strong className="text-indigo-700">{selectedScript.campaign?.name || 'N/A (Optional)'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Language:</span> <strong className="text-purple-700">{selectedScript.language}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Category / Purpose:</span> <strong className="text-amber-800">{selectedScript.category}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Objective:</span> <strong className="text-cyan-700">{selectedScript.objective || 'N/A'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Est. Duration:</span> <strong className="text-cyan-700">{selectedScript.estimatedDuration || 'N/A'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Created By:</span> <strong className="text-slate-800">{selectedScript.createdBy?.name || 'Writer'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Created At:</span> <strong className="text-slate-700">{new Date(selectedScript.createdAt).toLocaleDateString()}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Remarks:</span> <strong className="text-amber-800">{selectedScript.remarks || 'None'}</strong></div>
                    <div><span className="text-slate-400 block text-[10px]">Total Revisions:</span> <strong className="text-amber-800 font-bold">{selectedScript.revisionCount || 0}</strong></div>
                  </div>
                )}
              </div>
              ...
              {isEditingLocked && (
                <div className="p-3.5 bg-amber-50 border border-amber-300 rounded-xl text-amber-900 text-xs font-semibold flex items-center gap-2.5 shadow-md">
                  ...
                  <div className="bg-slate-50/90 border border-slate-200 rounded-xl p-3 space-y-1">
                    ...
                    <div>
                      <span className={`inline-block px-2 py-0.5 rounded font-mono font-bold text-[11px] ${
                        (selectedScript.priority || editPriority) === 'CRITICAL' ? 'bg-rose-50 text-rose-700 border border-rose-200' :
                        (selectedScript.priority || editPriority) === 'HIGH' ? 'bg-amber-50 text-amber-800 border border-amber-200' :
                        (selectedScript.priority || editPriority) === 'LOW' ? 'bg-slate-50 text-slate-700 border border-slate-200' :
                ...
                <div className="p-4 bg-slate-50 border border-purple-200 rounded-2xl space-y-3 shadow-lg">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {copiedStoryline ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3 text-purple-600" />}
                      ...
                      <div className="flex bg-slate-50 border border-slate-200 p-0.5 rounded-lg text-[10px] font-semibold">
                        <button
                          ...
                          className={`px-2 py-0.5 rounded transition-colors ${storylineTab === 'view' ? 'bg-purple-600 text-white font-bold' : 'text-slate-500 hover:text-slate-900'}`}
                        >
                          Formatted View
                        </button>
                        <button
                          ...
                          className={`px-2 py-0.5 rounded transition-colors ${storylineTab === 'edit' ? 'bg-purple-600 text-white font-bold' : 'text-slate-500 hover:text-slate-900'}`}
                        >
                          Edit Storyline
                        </button>
                      </div>
                  ...
                  {storylineTab === 'view' ? (
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full bg-slate-50 border border-purple-200 text-slate-900 p-3 rounded-xl text-xs font-mono focus:outline-none focus:border-purple-500 focus:bg-white disabled:opacity-50 disabled:cursor-not-allowed"
                      />
              ...
              <div className="p-4 bg-slate-50 border border-emerald-200 rounded-xl space-y-3">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                  ...
                  <span className="text-[10px] text-slate-400 italic">Entries are permanent and never deleted</span>
                </div>

                {(selectedScript.timeline || []).length === 0 ? (
                  <p className="text-slate-400 italic text-[11px]">No timeline events yet</p>
                ) : (
                    ...
                    {(selectedScript.timeline || []).map((t: any, idx: number) => {
                      ...
                      return (
                        <div key={t.id} className="flex items-start gap-3">
                          <div className={`w-3.5 h-3.5 rounded-full ${dotColor} shrink-0 mt-0.5 border-2 border-slate-200 z-10`} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="font-bold text-slate-900 text-[11px]">{label}</span>
                            ...
                            {t.triggeredBy && (
                              <p className="text-[10px] text-slate-400 mt-0.5">By: {t.triggeredBy.name}</p>
                            )}
                          </div>
                        </div>
                      );
              ...
              <div className="p-4 bg-slate-50 border border-amber-200 rounded-xl space-y-3">
                ...
                <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                  {(selectedScript.scriptRemarks || []).length === 0 ? (
                    <p className="text-slate-400 italic text-[11px]">No remarks yet. Be the first to add one.</p>
                  ) : (
                    (selectedScript.scriptRemarks || []).map((r: any) => {
                      const date = new Date(r.createdAt);
                      return (
                        <div key={r.id} className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 space-y-1">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5">
                          ...
                          catLinks.map((l: any) => (
                            <div key={l.id} className="flex items-center justify-between gap-1 bg-slate-50 p-1.5 rounded border border-slate-200 group">
                              <a
                                ...
                                className="text-purple-700 hover:text-purple-100 font-mono text-[10px] truncate underline underline-offset-2 flex-1"
                                title={l.url}
                              >
                                {l.name}
                              </a>
              ...
              <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl space-y-3 shadow-lg">
                <div className="flex items-center justify-between border-b border-purple-200 pb-2">
                  ...
                  <span className="text-[10px] bg-purple-50 text-purple-200 border border-purple-300 px-2 py-0.5 rounded font-mono font-bold">
                    Current Status: {selectedScript.status}
                  </span>
                </div>

                {selectedScript.status === 'WAITING_FOR_TECHNICAL_REVIEW' ? (
                  <div className="p-3 bg-blue-50 border border-blue-300 rounded-lg space-y-2 text-blue-200">
                    <div className="flex items-center justify-between">
                  ...
                  <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg space-y-1 text-emerald-200">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className={`p-3 rounded-lg border text-xs space-y-1 ${
                                isApproved
                                  ? 'bg-emerald-900/30 border-emerald-300 text-emerald-200'
                                  : 'bg-red-900/30 border-rose-300 text-red-200'
                              }`}>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <form
            ...
            className="bg-slate-50 border border-slate-200 p-6 rounded-2xl max-w-2xl w-full space-y-4 max-h-[90vh] overflow-y-auto text-xs"
          >
            <div className="flex justify-between items-center border-b border-slate-200 pb-3">
              <div>
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  <FileText className="w-4 h-4 text-purple-600" /> Create New Production Script
                </h2>
                <p className="text-[11px] text-slate-500">Define script parameters and operational requirements</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {/* 3. Shoot Project */}
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Target Shoot Project *</label>
                <select
                  value={newProjectId}
              ...
              {/* 2. Script Name / Code */}
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Script Name / Custom Code</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Script Purpose / Category *</label>
                <select
                  value={newCategory}
            ...
            <div className="bg-slate-50/80 border border-purple-200 p-4 rounded-xl space-y-2.5">
              <div className="flex items-center justify-between">
              ...
              <div
                onClick={() => setShowDescriptionPopup(true)}
                className="p-3 bg-slate-50/90 border border-slate-200 hover:border-purple-200 rounded-xl cursor-pointer transition-all group"
              >
                {newDescription?.trim() ? (
                  <div className="space-y-1">
                    <p className="text-slate-800 text-xs font-sans line-clamp-3 leading-relaxed whitespace-pre-wrap">
                      {newDescription}
                    </p>
                    <div className="flex justify-between items-center text-[10px] text-purple-600 font-mono pt-1.5 border-t border-slate-200">
                      <span>{newDescription.split('\n').filter(Boolean).length} lines • {newDescription.length} characters</span>
                      <span className="text-purple-700 group-hover:underline font-semibold flex items-center gap-1">
                        Click to edit in popup
                      </span>
                    </div>
                  </div>
                ) : (
                  <div className="py-2 text-center text-slate-400 text-xs italic flex flex-col items-center gap-1">
                    <span>Click here to open popup &amp; write full script storyline, scenes, voiceovers &amp; shot list</span>
                    <span className="px-2.5 py-0.5 rounded bg-purple-50 text-purple-700 border border-purple-200 text-[10px] font-mono not-italic mt-1">
      ...
      {showDescriptionPopup && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs backdrop-blur-xs flex items-center justify-center p-4 z-[60] animate-in fade-in duration-150">
          <div className="bg-slate-50 border border-purple-200 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col p-6 space-y-4 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3 shrink-0">
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 flex items-center justify-center font-bold text-sm transition-colors border border-slate-200"
              >
                ×
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs shrink-0">
              <span className="text-slate-500 text-[11px]">Write scenes, visual instructions, and voiceover dialogues</span>
              <div className="flex items-center gap-2 text-[10px]">
                <button
                  ...
                  className="px-2.5 py-1 bg-purple-50 hover:bg-purple-900 text-purple-700 border border-purple-200 rounded-lg font-semibold transition-colors"
                >
                  + Add Scene Template
                </button>
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg font-semibold flex items-center gap-1 transition-colors"
                >
            ...
            <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
              {!showStorylinePreview ? (
                ...
                <div className="bg-slate-50 border border-purple-200 p-4 rounded-xl flex-1 max-h-[55vh] overflow-y-auto custom-scrollbar break-words [overflow-wrap:anywhere]">
                  {newDescription?.trim() ? (
                    <div className="whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-sans text-slate-800 text-xs leading-relaxed">
                      {newDescription}
                    </div>
                  ) : (
                    <span className="text-slate-400 italic text-xs">No storyline text entered yet. Switch to Edit Text to add content.</span>
                  )}
                </div>
              )}
            </div>
      ...
      {viewingScriptDescription && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs backdrop-blur-xs flex items-center justify-center p-4 z-[60] animate-in fade-in duration-150">
          <div className="bg-slate-50 border border-purple-200 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col p-6 space-y-4 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3 shrink-0">
              <div className="flex items-center gap-2">
                <FileText className="w-5 h-5 text-purple-600" />
                <div>
                  ...
                  <span className="text-[10px] text-purple-700 font-mono">ID: {viewingScriptDescription.scriptId}</span>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex-1 max-h-[55vh] overflow-y-auto custom-scrollbar break-words [overflow-wrap:anywhere]">
              {viewingScriptDescription.description?.trim() ? (
                <div className="whitespace-pre-wrap break-words [overflow-wrap:anywhere] font-sans text-slate-800 text-xs leading-relaxed tracking-wide">
                  {viewingScriptDescription.description}
                </div>
              ) : (
                <p className="text-slate-400 italic text-xs text-center py-4">No storyline text recorded for this script.</p>
              )}
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-200 shrink-0">
              <button
                ...
                className="px-3 py-1.5 bg-purple-50 hover:bg-purple-900 border border-purple-200 text-purple-700 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors"
              >
                {copiedStoryline ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4 text-purple-600" />}
                <span>{copiedStoryline ? 'Copied to Clipboard!' : 'Copy Script Text'}</span>
              </button>
              <button
                type="button"
