    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-200 p-6 rounded-xl">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <Package className="w-5 h-5 text-blue-600" /> Brand Products & Codes Catalog
          </h1>
        </div>
              ...
              {safeProducts.map((prod) => (
                <tr key={prod.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="p-4">
                    ...
                    <span className="font-bold text-slate-900 text-sm block">{prod.name}</span>
                  </td>

                  <td className="p-4">
                    <div className="font-semibold text-slate-800">[{prod.brand?.shortCode}] {prod.brand?.name}</div>
                    <div className="text-[10px] text-slate-500">{prod.brand?.client?.name}</div>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    {((user?.role as string) === 'MARKETING_MANAGER' || (user?.role as string) === 'ADMIN' || (user?.role as string) === 'ADMINISTRATOR') && (
                      <button
                        ...
                        className="px-2 py-1 bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-600/30 rounded font-semibold text-[11px] inline-flex items-center gap-1"
                      >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-xl w-full max-w-md p-6 space-y-4 text-xs">
            ...
            {!editingProduct && (
              <div>
                <label className="text-slate-500 block mb-1 font-semibold">Parent Active Brand *</label>
                <select
                  required
                ...
                {activeBrands.length === 0 && (
                  <p className="text-[10px] text-amber-600 mt-1">No active brands available! Create an active brand first.</p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-slate-500 block mb-1 font-semibold">Product Name *</label>
                <input
                  type="text"
                  required
                  value={formData.name}
              ...
              <div>
                <label className="text-slate-500 block mb-1 font-semibold">Product Code (Unique in Brand) *</label>
                <input
                  type="text"
                  required
                  maxLength={6}
