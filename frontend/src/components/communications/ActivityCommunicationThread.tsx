...
export default function ActivityCommunicationThread({
  ...
  const renderAttachmentItem = (att: any) => {
      ...
      return (
        <div key={att.id || att.fileUrl} className="group relative bg-white border border-slate-200 rounded-lg overflow-hidden p-1 space-y-1 shadow-xs">
          ...
          <div className="flex items-center justify-between text-[10px] text-slate-700 px-1 truncate">
            ...
            <a href={att.fileUrl} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-800 shrink-0"><Download className="w-3 h-3" /></a>
          </div>
        </div>
      );
    }
    ...
    if (isVid) {
      return (
        <div key={att.id || att.fileUrl} className="bg-white border border-slate-200 rounded-lg p-2 space-y-1 shadow-xs">
          ...
          <video src={att.fileUrl} controls className="w-full h-28 object-cover rounded bg-slate-900" />
        </div>
      );
    }
    ...
    return (
      <div key={att.id || att.fileUrl} className="flex items-center justify-between bg-white border border-slate-200 p-2 rounded-lg text-xs shadow-xs">
        ...
        <a href={att.fileUrl} target="_blank" rel="noreferrer" className="px-2 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded text-[10px] font-semibold flex items-center gap-1 shrink-0">
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 text-xs shadow-xs">
      ...
      <form onSubmit={handleSubmitRoot} className="border-t border-slate-100 pt-3 space-y-2.5">
        <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-1.5 rounded-xl">
          <span className="text-[11px] text-slate-700 font-semibold px-1">Entry Type:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-slate-900 text-xs focus:outline-none focus:border-blue-500 focus:bg-white transition-colors"
            />
          </div>
          ...
          {entryMode === 'COMMUNICATION' ? (
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs rounded-lg p-1.5 focus:outline-none focus:border-blue-500 focus:bg-white font-semibold transition-colors"
              >
                {categories.map((cat) => (
            ...
            <div className="space-y-1 opacity-60">
              <label className="text-[10px] text-slate-400 font-semibold uppercase">Responses / Replies:</label>
              <input
                type="text"
                disabled
                value="Disabled for Remarks"
                className="w-full bg-slate-100 border border-slate-200 text-slate-400 rounded-lg p-1.5 text-xs cursor-not-allowed"
              />
            </div>
          )}
          ...
          {entryMode === 'COMMUNICATION' && type === 'ANNOUNCEMENT' ? (
            <div className="space-y-1">
              <label className="text-[10px] text-purple-700 font-semibold uppercase flex items-center gap-1">
                <Bell className="w-3 h-3 text-purple-600" /> Announcement Priority:
              </label>
              <select
                value={priority}
                onChange={(e: any) => setPriority(e.target.value)}
                className="w-full bg-purple-50 border border-purple-200 text-purple-900 text-xs font-semibold rounded-lg p-1.5 focus:outline-none focus:bg-white"
              >
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-rose-700 font-semibold uppercase flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" /> Blocker Reason Category:
              </label>
              <select
                value={blockerReason}
                onChange={(e) => setBlockerReason(e.target.value)}
                className="w-full bg-rose-50 border border-rose-200 text-rose-900 text-xs font-semibold rounded-lg p-1.5 focus:outline-none focus:bg-white"
              >
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-emerald-700 font-semibold uppercase flex items-center gap-1">
                <ShieldCheck className="w-3 h-3" /> Target Review Manager:
              </label>
              <select
                value={targetRole}
                onChange={(e: any) => setTargetRole(e.target.value)}
                className="w-full bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-semibold rounded-lg p-1.5 focus:outline-none focus:bg-white"
              >
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-slate-500 font-semibold uppercase">Recipient(s) (Mandatory):</label>
              <input
                type="text"
            ...
            <div className="space-y-1 opacity-60">
              <label className="text-[10px] text-slate-400 font-semibold uppercase">Recipient(s):</label>
              <input
                type="text"
                disabled
        ...
        {entryMode === 'COMMUNICATION' && (type === 'BLOCKER' || type === 'ISSUE_REPORT') && (
          <div className="space-y-1 bg-rose-50/70 p-2.5 rounded-xl border border-rose-200">
            <label className="text-[10px] text-purple-700 font-semibold uppercase flex items-center gap-1">
            ...
            <select
              ...
              className="w-full bg-white border border-slate-300 text-slate-900 text-xs rounded-lg p-1.5 focus:outline-none focus:border-purple-500 font-medium"
            >
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-1 text-[10px]">
            ...
            {staffList.map((staff) => (
              <button
                ...
                className="px-2 py-0.5 bg-slate-100 hover:bg-blue-50 hover:text-blue-700 border border-slate-200 rounded text-slate-700 transition-colors whitespace-nowrap font-medium"
              >
                @{staff.name.split(' ')[0]}
              </button>
            ))}
        ...
        {attachments.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap bg-slate-50 p-2 rounded-lg border border-slate-200 text-[11px]">
            ...
            {attachments.map((att, idx) => (
              <span key={idx} className="bg-white text-slate-700 px-2 py-0.5 rounded flex items-center gap-1 border border-slate-200 shadow-xs">
                <span className="text-purple-700 font-mono text-[10px]">[{att.fileType}]</span> {att.fileName}
                <X className="w-3 h-3 cursor-pointer hover:text-rose-600 ml-1" onClick={() => setAttachments((prev) => prev.filter((_, i) => i !== idx))} />
              </span>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-2">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-2 text-slate-900 text-xs focus:outline-none focus:border-blue-500 focus:bg-white placeholder-slate-400 resize-none transition-colors"
          />

          <button
            type="button"
            onClick={() => setShowAttachModal(true)}
            className="px-3 py-2 bg-white hover:bg-slate-50 text-purple-700 border border-purple-200 rounded-xl text-xs font-semibold flex items-center gap-1.5 self-end transition-colors shadow-xs"
          >
            <Paperclip className="w-3.5 h-3.5 text-purple-600" /> Attach File
          </button>
          ...
          <button
            ...
            } disabled:opacity-50 text-white font-semibold rounded-xl flex items-center gap-1.5 self-end text-xs transition-colors shadow-xs`}
          >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-5 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              ...
              <button onClick={() => setShowAttachModal(false)} className="text-slate-400 hover:text-slate-700">
                <X className="w-4 h-4" />
              </button>
            </div>
