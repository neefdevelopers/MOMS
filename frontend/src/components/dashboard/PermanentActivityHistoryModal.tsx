    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
        ...
        <div className="p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono font-black uppercase px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs font-bold px-3 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 flex items-center gap-1.5 transition-all shadow-xs disabled:opacity-50"
            >
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 bg-slate-50/70 border-b border-slate-200 shrink-0">
            <div className="p-2.5 bg-white border border-slate-200 rounded-xl shadow-xs">
              <span className="text-[10px] font-bold text-slate-500 uppercase block">Total Audit Logs</span>
              <span className="text-lg font-black text-slate-900">{stats.totalLogs || 0}</span>
            </div>
            <div className="p-2.5 bg-white border border-slate-200 rounded-xl shadow-xs">
              <span className="text-[10px] font-bold text-emerald-700 uppercase block">24h Operational Events</span>
              <span className="text-lg font-black text-emerald-700">{stats.logs24h || 0}</span>
            </div>
            <div className="p-2.5 bg-white border border-slate-200 rounded-xl shadow-xs">
              <span className="text-[10px] font-bold text-blue-700 uppercase block">Projects Logged</span>
              <span className="text-lg font-black text-blue-700">{stats.entityCounts?.PROJECT || 0}</span>
            </div>
            <div className="p-2.5 bg-white border border-slate-200 rounded-xl shadow-xs">
              <span className="text-[10px] font-bold text-amber-700 uppercase block">Tasks & Reviews Logged</span>
        ...
        <div className="p-4 bg-white border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 shrink-0">
          <form onSubmit={handleSearchSubmit} className="relative flex-1 min-w-[240px]">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-xs rounded-xl pl-9 pr-3 py-2 outline-none focus:bg-white focus:border-emerald-500"
            />
          </form>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-xl px-3 py-2 outline-none focus:bg-white focus:border-emerald-500"
            >
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500">
              <input
                ...
                className="bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-xl px-2.5 py-1.5 outline-none"
              />
              <span>to</span>
              <input
                type="date"
        ...
        <div className="p-5 overflow-y-auto flex-1 bg-white">
          {loading ? (
                  ...
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-3 font-medium whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-slate-200 border border-slate-300 flex items-center justify-center text-[9px] font-bold text-slate-700 uppercase">
                      ...
                      <td className="p-3 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-slate-100 border border-slate-200 font-mono text-[10px] font-bold text-slate-700 uppercase">
                          {getEntityIcon(log.entity)}
                          {log.entity}
                        </span>
                      </td>

                      <td className="p-3 font-mono text-[10px] font-bold text-amber-700 whitespace-nowrap">
                        {log.action}
                      </td>

                      <td className="p-3 max-w-md truncate text-slate-700">
                        {log.description}
                      </td>
                      ...
                      <td className="p-3 text-right whitespace-nowrap">
                        <button
                          ...
                          className="text-[11px] font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-2.5 py-1 rounded-lg transition-all shadow-xs"
                        >
                          Inspect JSON
                        </button>
                      </td>
            ...
            <div className="py-16 text-center bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              ...
              <h3 className="text-base font-bold text-slate-800">No Matching Permanent Logs</h3>
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-60 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                <Activity className="w-4 h-4 text-emerald-600" /> Permanent Activity Log Inspection
              </h3>
            ...
            <div className="space-y-2 text-xs font-mono text-slate-700">
              <div><strong className="text-slate-500">Log ID:</strong> <span className="text-slate-900">{selectedLog.id}</span></div>
              <div><strong className="text-slate-500">Timestamp:</strong> <span className="text-slate-900">{new Date(selectedLog.timestamp).toISOString()}</span></div>
              <div><strong className="text-slate-500">Action:</strong> <span className="text-amber-700 font-bold">{selectedLog.action}</span></div>
              <div><strong className="text-slate-500">Entity:</strong> <span className="text-blue-700">{selectedLog.entity} ({selectedLog.entityId})</span></div>
              <div><strong className="text-slate-500">User:</strong> <span className="text-slate-900">{selectedLog.user?.name} ({selectedLog.user?.email})</span></div>
              <div><strong className="text-slate-500">Description:</strong> <span className="text-slate-800">{selectedLog.description}</span></div>
            </div>

            {selectedLog.metadata && (
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500 block font-mono">Raw Event Metadata (JSON):</label>
                <pre className="bg-slate-50 p-3 rounded-xl text-[11px] font-mono text-emerald-800 border border-slate-200 overflow-x-auto max-h-48">
                  {(() => {
                    try {
